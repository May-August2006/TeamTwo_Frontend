...
const BulkMeterReadingPage: React.FC = () => {
  ...
  const [unitsWithMonthlyReadings, setUnitsWithMonthlyReadings] = useState<
    Set<number>
  >(new Set());
  ...
  const downloadCSVTemplate = () => {
    ...
    a.href = url;
    a.download = `meter-readings-template-${
  ...
  const processCSVData = async (csvText: string): Promise<any[]> => {
    ...
    console.log(
      "Occupied units with meters:",
      occupiedUnits.map((u) => ({ id: u.id, number: u.unitNumber }))
    );
    ...
    for (const [index, row] of dataRows.entries()) {
      ...
      if (columns.length >= 3) {
        ...
        console.log(
          `Processing row ${
            index + 2
          }: Unit ${unitNumber}, Elec: ${electricityReading}, Water: ${waterReading}`
        );
        ...
        if (!unit) {
          errors.push(
            `Row ${
              index + 2
            }: Unit ${unitNumber} not found or doesn't have meters`
          );
          continue;
        }
        ...
        if (unitsWithMonthlyReadings.has(unit.id)) {
          errors.push(
            `Row ${
              index + 2
            }: Unit ${unitNumber} already has readings for this month`
          );
          continue;
        }
        ...
        console.log(
          `Added unit ${unitNumber} (ID: ${unit.id}) to processed readings`
        );
      }
    }
    ...
    try {
      ...
      const bulkRequests = processedReadings.flatMap((reading) => {
        ...
        if (electricityUtility && reading.electricityReading >= 0) {
          // Changed > to >=
          console.log(
            `Adding electricity reading for unit ${reading.unitId}: ${reading.electricityReading}`
          );
          requests.push({
        ...
        if (waterUtility && reading.waterReading >= 0) {
          // Changed > to >=
          console.log(
            `Adding water reading for unit ${reading.unitId}: ${reading.waterReading}`
          );
          requests.push({
            unitId: reading.unitId,
      ...
      if (bulkRequests.length === 0) {
        alert(
          "No valid readings to submit from CSV. Check if utilities are configured and readings are valid."
        );
        return;
      }
  ...
  const submitBulkReadings = async () => {
    ...
    if (invalidReadings.length > 0) {
      alert(
        `Found ${invalidReadings.length} readings where current reading is less than previous reading. Please check the values.`
      );
      return;
    }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Enter meter readings for all units in a building at once
          </p>
        ...
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            ...
            <div className="flex items-end space-x-2">
              ...
              <label
                className={`px-4 py-2 border border-gray-300 rounded hover:bg-gray-50 flex items-center cursor-pointer ${
                  loading || !buildingId ? "opacity-50 cursor-not-allowed" : ""
                }`}
              >
              ...
              {unitsWithMonthlyReadings.size > 0 && (
                <div className="mt-3 p-2 bg-yellow-50 border border-yellow-200 rounded">
                  <p className="text-sm text-yellow-800">
                    ⚠️ {unitsWithMonthlyReadings.size} units already have
                    readings for this month and are disabled
                  </p>
                </div>
              )}
            </div>
              ...
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    ...
                    {unitsWithMonthlyReadings.size > 0 &&
                      ` (${unitsWithMonthlyReadings.size} disabled)`}
                  </p>
                </div>
                <div className="text-sm">
                  ...
                  <span className="font-medium ml-2">
                    {bulkReadings
                      .reduce(
                        (sum, r) => sum + r.electricityReading + r.waterReading,
                        0
                      )
                      .toFixed(2)}
                  </span>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {bulkReadings.map((reading) => {
                    ...
                    const isDisabled = unitsWithMonthlyReadings.has(
                      reading.unitId
                    );
              ...
              <div className="flex justify-between items-center">
                <div className="text-sm text-gray-600">
                  ...
                  <div className="mt-1">
                    Total Electricity:{" "}
                    {bulkReadings
                      .reduce((sum, r) => sum + r.electricityReading, 0)
                      .toFixed(2)}{" "}
                    kWh
                  </div>
                  <div>
                    Total Water:{" "}
                    {bulkReadings
                      .reduce((sum, r) => sum + r.waterReading, 0)
                      .toFixed(2)}{" "}
                    gal
                  </div>
                </div>
                <div className="flex space-x-2">
                  ...
                  <button
                    ...
                    disabled={
                      loading ||
                      bulkReadings.filter(
                        (r) => !unitsWithMonthlyReadings.has(r.unitId)
                      ).length === 0
                    }
