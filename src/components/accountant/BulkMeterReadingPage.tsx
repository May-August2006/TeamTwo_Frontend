...
const BulkMeterReadingPage: React.FC = () => {
  ...
  const [electricityUtility, setElectricityUtility] =
  ...
  const checkReadingStatus = useCallback(
      ...
      try {
        ...
        const updatedReadings = await Promise.all(
          bulkReadings.map(async (reading) => {
            ...
            let previousElectricityReading =
              reading.previousElectricityReading || 0;
            ...
            try {
              ...
              console.log(
                `No previous electricity reading for unit ${reading.unitId}`
              );
            ...
            try {
              ...
              console.log(
                `No previous water reading for unit ${reading.unitId}`
              );
            ...
            try {
              ...
              if (error.response?.status !== 401) {
                console.log(
                  `No electricity reading for unit ${reading.unitId}`
                );
              }
            }
            ...
            try {
              ...
              console.log(
                `Unit ${reading.unitId} - Previous electricity:`,
                electricityPrev
              );
              previousElectricityReading = electricityPrev?.currentReading || 0;
            } catch (error: any) {
  ...
  const checkBuildingReadingsStatus = useCallback(async () => {
    ...
    try {
      ...
      const occupiedUnitsResponse =
        await buildingApi.getOccupiedUnitsByBuilding(buildingId);
      const occupiedUnitsData = occupiedUnitsResponse.data || [];

      // Filter only ROOM and HALL type units with meters
      const unitsWithMeters = occupiedUnitsData.filter(
        (unit: Unit) =>
          unit.hasMeter &&
          (unit.unitType === "ROOM" || unit.unitType === "HALL")
      );
  ...
  const loadData = async () => {
    try {
      ...
      try {
        const assignedBuildingResponse =
          await buildingApi.getMyAssignedBuilding();
  ...
  const handleBuildingChange = async (selectedBuildingId: number) => {
    ...
    if (
      assignedBuildingId &&
      selectedBuildingId !== assignedBuildingId &&
      !isAdmin
    ) {
      setUploadStatus({
        type: "error",
        message:
          "You can only access meter readings for your assigned building",
      });
      return;
    }
    ...
    try {
      ...
      const occupiedUnitsResponse =
        await buildingApi.getOccupiedUnitsByBuilding(selectedBuildingId);
      const occupiedUnitsData = occupiedUnitsResponse.data || [];
      ...
      const initialReadings: BulkReading[] = unitsWithMeters.map(
        (unit: Unit) => ({
          unitId: unit.id,
      ...
      prev.map((reading) => {
        if (reading.unitId === unitId) {
          ...
          if (
            ...
            reading.previousElectricityReading &&
            numValue < reading.previousElectricityReading
          ) {
            setUploadStatus({
              type: "warning",
          ...
          if (
            field === "waterReading" &&
            reading.previousWaterReading &&
            numValue < reading.previousWaterReading
          ) {
            setUploadStatus({
              type: "warning",
  ...
  const downloadExcelTemplate = () => {
    ...
    if (buildingHasAllReadings) {
      setUploadStatus({
        type: "warning",
        message:
          "All units in this building already have readings for this month. Excel template download is disabled.",
      });
      return;
    }

    const buildingName =
      buildings.find((b) => b.id === buildingId)?.buildingName || "Building";
    ...
    const rows = buildingUnits.map((unit) => [unit.unitNumber, "", "", ""]);

    const sheetData = [...instructions, ...headers, ...rows];
    const worksheet = XLSX.utils.aoa_to_sheet(sheetData);

    worksheet["!cols"] = [{ wch: 20 }, { wch: 32 }, { wch: 30 }, { wch: 30 }];
    ...
    XLSX.writeFile(
      workbook,
      `Meter_Readings_Template_${buildingName.replace(
        /\s+/g,
        "_"
      )}_${readingDate}.xlsx`
    );
  ...
  const validateExcelFormat = (sheetData: any[][]): boolean => {
    ...
    const headerRow = sheetData.find((row) => row.includes("Unit Number"));
  ...
  const processExcelData = async (sheetData: any[][]): Promise<any[]> => {
    ...
    dataRows.forEach((row, index) => {
      ...
      if (existingReading?.hasMonthlyReading) {
        errors.push(
          `Row ${index + headerIndex + 2}: Already has reading for this month`
        );
        return;
      }
  ...
  const handleFileUpload = async (
    ...
    if (buildingHasAllReadings) {
      setUploadStatus({
        type: "warning",
        message:
          "All units already have readings for this month. Excel import is disabled.",
      });
      event.target.value = "";
      return;
    }
  ...
  const submitReadings = async (
    ...
    try {
      const bulkRequests = readingsToSubmit.flatMap((reading) => {
        ...
        const fullReading = bulkReadings.find(
          (r) => r.unitId === reading.unitId
        );
        if (!fullReading) return [];

        // Electricity reading - only submit if it doesn't already exist
        if (
          !fullReading.hasElectricityReading &&
          reading.electricityReading >= 0
        ) {
          requests.push({
  ...
  const submitBulkReadings = async () => {
    ...
    const invalidReadings = unitsToSubmit.filter((reading) => {
      const elecConsumption =
        reading.electricityReading - (reading.previousElectricityReading || 0);
      const waterConsumption =
        reading.waterReading - (reading.previousWaterReading || 0);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Enter meter readings for ROOM and HALL type units in a building at
            once
          </p>
        ...
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Readings are recorded monthly. Once submitted, fields will be
                disabled.
              </p>
            </div>
          ...
          {buildingId && (
            <div className="mt-4 p-4 bg-blue-50 rounded border border-blue-200">
              ...
              {buildingHasAllReadings && (
                <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded">
                  <p className="text-sm text-green-800 flex items-center gap-2">
                    ...
                    All units in this building have readings for {readingDate}.
              ...
              {!buildingHasAllReadings &&
                bulkReadings.some((r) => r.hasMonthlyReading) && (
                  <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded">
                    <p className="text-sm text-yellow-800 flex items-center gap-2">
                      ...
                      unit(s) already have readings for {readingDate}. These are
                      disabled to prevent duplicate entries.
                    </p>
                  </div>
                )}
            </div>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {bulkReadings.map((reading) => {
                    ...
                    const waterConsumption =
                      ...
                      (reading.previousWaterReading || 0);

                    return (
                      <tr
                        key={reading.unitId}
                        className={`hover:bg-gray-50 ${
                          reading.isDisabled ? "bg-gray-50" : ""
                        }`}
                      >
                          ...
                          <div className="space-y-2">
                            <div className="flex items-center gap-2">
                              <input
                                ...
                                title={
                                  reading.hasElectricityReading
                                    ? "Reading already submitted for this month"
                                    : ""
                                }
                              />
                            </div>
                            <div className="text-xs text-gray-500">
                              Previous:{" "}
                              {reading.previousElectricityReading?.toFixed(2) ||
                                "0.00"}{" "}
                              kWh
                            </div>
                          </div>
                        ...
                        <td className="px-6 py-4">
                          <div className="space-y-2">
                            ...
                            <div className="text-xs text-gray-500">
                              Previous:{" "}
                              {reading.previousWaterReading?.toFixed(2) ||
                                "0.00"}{" "}
                              gal
                            </div>
                          </div>
                        ...
                        <td className="px-6 py-4 text-sm text-gray-600">
                          <div className="space-y-2">
                            <div className="p-2 bg-gray-50 rounded">
                              ...
                              <div>
                                {reading.previousElectricityReading?.toFixed(
                                  2
                                ) || "0.00"}{" "}
                                kWh
                              </div>
                        ...
                        <td className="px-6 py-4">
                          <div className="space-y-3">
                            ...
                            <div>
                              ...
                              >
                                {waterConsumption >= 0 ? "+" : ""}
                                {waterConsumption.toFixed(2)} gal
                              </div>
                            </div>
                          </div>
                        </td>
