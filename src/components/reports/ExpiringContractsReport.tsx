...
export const ExpiringContractsReport: React.FC<
...
> = ({ onBack }) => {
  ...
  const getBusinessType = (contract: Contract): string => {
    ...
    if (contract.tenant?.tenantName) {
      ...
      if (
        tenantName.includes("restaurant") ||
        tenantName.includes("cafe") ||
        tenantName.includes("food")
      ) {
        return "Food & Beverage";
      } else if (
        tenantName.includes("retail") ||
        tenantName.includes("shop") ||
        tenantName.includes("store")
      ) {
        return "Retail";
      } else if (
        tenantName.includes("office") ||
        tenantName.includes("service")
      ) {
        return "Office/Service";
      } else if (
        tenantName.includes("beauty") ||
        tenantName.includes("salon") ||
        tenantName.includes("spa")
      ) {
        return "Beauty & Wellness";
      } else if (
        tenantName.includes("clinic") ||
        tenantName.includes("medical") ||
        tenantName.includes("health")
      ) {
        return "Medical";
      } else if (
        tenantName.includes("bank") ||
        tenantName.includes("finance") ||
        tenantName.includes("money")
      ) {
        return "Financial";
      }
    }
  ...
  const stats = useMemo(() => {
    ...
    const expiringSoon = expiringContracts.filter(
      (c) => c.status === "EXPIRING_SOON"
    ).length;
    const expired = expiringContracts.filter(
      (c) => c.status === "EXPIRED"
    ).length;
    const needsRenewal = expiringContracts.filter(
      (c) => c.status === "NEEDS_RENEWAL"
    ).length;
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(
      workbook,
      `expiring-contracts-${new Date().toISOString().split("T")[0]}.xlsx`
    );
  };
        ...
        <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
          <div className="flex items-center">
            ...
            <span className="text-orange-800 font-medium">
              {stats.expiringSoon} contract(s) expiring within 30 days require
              attention
            </span>
          </div>
        </div>
      ...
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        ...
        <div className="mt-4 pt-4 border-t border-gray-200">
          <p className="text-sm text-gray-600">
            Showing {expiringContracts.length} contract(s) matching your
            criteria
          </p>
        </div>
      </div>
                  ...
                  >
                    <td className="px-6 py-4">
                      ...
                      <div className="text-sm text-gray-500">
                        Rental: {formatCurrency(contract.rentalFee)}
                      </div>
                      <div className="text-sm text-gray-500">
                        {contract.businessType}
                      </div>
