...
export const userApi = {
  ...
  update: (id: number, user: UserRequest) => API.put<User>(`/api/users/${id}`, user),
  delete: (id: number) => API.delete<void>(`/api/users/${id}`),
  getByRole: (roleName: string) => API.get<User[]>(`/api/users/role/${roleName}`),
  getByRoles: (roleNames: string[]) => API.get<User[]>(`/api/users/by-roles?roleNames=${roleNames.join(',')}`),
  ...
  getAvailableAccountants: () => API.get<User[]>('/api/users/accountants/available'),
  checkUsername: (username: string) => API.get<{available: boolean}>(`/api/users/check-username/${username}`),
  checkEmail: (email: string) => API.get<{available: boolean}>(`/api/users/check-email/${email}`),
};