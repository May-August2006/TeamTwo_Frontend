...
export const paymentApi = {
  ...
  async recordLateFeesPayment(paymentData: PaymentRequest): Promise<Payment> {
    const response = await API.post<Payment>(
      "/api/payments/late-fee",
      paymentData
    );
    return response.data;
  },
  ...
  voidPayment: async (
    ...
    const response = await API.put(
      `/api/payments/${paymentId}/void?reason=${encodeURIComponent(
        reason
  ...
  async getAuditLogsByPaymentId(paymentId: number): Promise<PaymentAuditLog[]> {
    const response = await API.get<PaymentAuditLog[]>(
      `/api/payment-audit-logs/payment/${paymentId}`
    );
    return response.data;
  },
  ...
  async getAuditLogsByUserId(userId: number): Promise<PaymentAuditLog[]> {
    const response = await API.get<PaymentAuditLog[]>(
      `/api/payment-audit-logs/user/${userId}`
    );
    return response.data;
  },
  ...
  async searchPaymentsByTenantName(tenantName: string): Promise<Payment[]> {
    const response = await API.get<Payment[]>(
      `/api/payments/search?tenantName=${encodeURIComponent(tenantName)}`
    );
    return response.data;
  },
  ...
  async getTotalPaidAmount(invoiceId: number): Promise<number> {
    const response = await API.get<number>(
      `/api/payments/invoice/${invoiceId}/total-paid`
    );
    return response.data;
  },
