...
const AccountantDashboard: React.FC = () => {
  ...
  const getSectionTitle = () => {
    ...
    switch (activeSection) {
      ...
      case 'utility-types': return "Utility Types";
      case 'billing-fees': return "Billing Fees";
      case 'usage-entry': return "Usage Entry";
      case 'bulk-readings': return "Bulk Meter Readings";
      case 'building-invoices': return "Building Utility Invoices";
      case 'payments': return "Payment Management";
      case 'invoices-management': return "Invoice Management";
      case 'late-fee': return "Late Fee Management";
      case 'overdue-outstanding': return "Overdue & Outstanding";
      ...
      <main className={`flex-grow transition-all duration-300 ${isSidebarCollapsed ? 'lg:ml-20' : 'lg:ml-64'}`}>
        ...
        <div className="p-6">
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            {/* Action Buttons */}
            {!showPaymentForm && (activeSection === 'overview' || activeSection === 'payment') && (
              <button
                onClick={() => setShowPaymentForm(true)}
                className="bg-red-600 text-white px-6 py-3 rounded-xl shadow-lg hover:bg-red-700 transition duration-150 font-semibold transform active:scale-95"
              >
            ...
            {!showPaymentForm && activeSection === 'utility-types' && (
              <button
                onClick={() => {/* Add utility type modal */}}
                className="bg-red-600 text-white px-6 py-3 rounded-xl shadow-lg hover:bg-red-700 transition duration-150 font-semibold transform active:scale-95"
              >
                + Add Utility Type
              </button>
            ...
            {!showPaymentForm && activeSection === 'bulk-readings' && (
              <button
                onClick={() => {/* Open bulk reading modal */}}
                className="bg-red-600 text-white px-6 py-3 rounded-xl shadow-lg hover:bg-red-700 transition duration-150 font-semibold transform active:scale-95"
              >
                + Add Bulk Reading
              </button>
            ...
            {!showPaymentForm && activeSection === 'building-invoices' && (
              <button
                onClick={() => {/* Generate building invoice */}}
                className="bg-red-600 text-white px-6 py-3 rounded-xl shadow-lg hover:bg-red-700 transition duration-150 font-semibold transform active:scale-95"
              >
                + Generate Invoice
              </button>
